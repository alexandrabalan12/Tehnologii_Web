...
const setUserRole = async (req, res) => {
  ...
  const is_valid_role = ["client", "company", "admin"].includes(role);

  if (!userId || !role || !is_valid_role) {
    res.statusCode = 400;
    res.write(
      JSON.stringify({ message: "User ID and valid role are required" })
    );
    res.end();
    return;
  }
  ...
  try {
    ...
    if (userResults.length === 0) {
      ...
      if (!isPasswordValid) {
        ...
        if (role === "company") {
          const [companyResults] = await database.query(companyQuery, [
            user.id,
          ]);
...
const authRoutes = (req, res) => {
  ...
  if (req.url.startsWith("/api/users/login") && req.method === "POST") {
    ...
    req.url.startsWith("/api/users/register") &&
    req.method === "POST"
  ) {
    let body = "";
    req.on("data", (chunk) => {

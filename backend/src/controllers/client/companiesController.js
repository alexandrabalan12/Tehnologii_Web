...
const createReview = async (req, res) => {
    ...
    if (!user_id || !company_id || !user_reviewer_name || !user_reviewer_occupation || rating === undefined || !review_content) {
        res.statusCode = 400;
    ...
    try {
        ...
        const selectCompanyQuery = 'SELECT COUNT(*) AS count FROM companies WHERE id = ?';
        ...
        `;
        const insertValues = [company_id, user_id, user_reviewer_name, user_reviewer_occupation, rating, review_content];
...
const clientCompaniesRoutes = (req, res) => {
    if(req.url.startsWith('/api/client/company-reviews/') && req.method === 'POST') {
        let body = '';
        req.on('data', chunk => {
            body += chunk;
        });
    ...
    } else if(req.url.startsWith('/api/client/company-details/') && req.method === 'GET') {
        protectedRoute(getCompanyDetails)(req, res);
    } else if(req.url.startsWith('/api/client/company-reviews/') && req.method === 'GET') {

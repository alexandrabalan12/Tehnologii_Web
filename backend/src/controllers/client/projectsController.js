...
const getProjectWithApplications = async (req, res) => {
  ...
  try {
    ...
    const selectApplicationsQuery = `
            ...
            WHERE ja.project_id = ?`;
    const [applicationRows] = await database.query(selectApplicationsQuery, [
      projectId,
    ]);
    const response = {
      project,
  ...
  try {
    ...
    if (projectIds.length > 0) {
      const selectApplicantsQuery = `
                ...
                GROUP BY project_id
            `;
      const [applicantCounts] = await database.query(
...
const closeProject = async (req, res) => {
  ...
  try {
    ...
    const [result] = await database.query(updateProjectQuery, [
      "closed",
      projectId,
    ]);
...
const clientProjectRoutes = (req, res) => {
  if (req.url.startsWith("/api/client/project-new") && req.method === "POST") {
    ...
    req.url.startsWith("/api/client/project-details") &&
    req.method === "GET"
  ) {
    protectedRoute(getProjectWithApplications)(req, res);
  } else if (
    req.url.startsWith("/api/client/projects") &&
    req.method === "POST"
  ) {
    let body = "";
    req.on("data", (chunk) => {
    ...
    req.url.startsWith("/api/client/project-close") &&
    req.method === "PUT"
  ) {

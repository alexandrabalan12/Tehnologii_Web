...
const createPortfolioProject = async (req, res) => {
  ...
  if (!company_id || !name || !professional_area || !description) {
    res.statusCode = 400;
  ...
  try {
    ...
    if (count === 0) {
      ...
      res.statusCode = 201;
      res.write(
        JSON.stringify({ message: "Portfolio project created successfully" })
      );
    }
    res.end();
  } catch (err) {
...
const deletePortfolioProjectById = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      ...
      res.statusCode = 200;
      res.write(
        JSON.stringify({ message: "Portfolio project deleted successfully" })
      );
    }
    res.end();
  } catch (err) {
...
const companyPortfolioRoutes = (req, res) => {
  if (
    req.url.startsWith("/api/company/portfolio-new") &&
    req.method === "POST"
  ) {
    let body = "";
    req.on("data", (chunk) => {
    ...
    req.url.startsWith("/api/company/portfolio-projects") &&
    req.method === "GET"
  ) {
    protectedRoute(getPortfolioProjects)(req, res);
  } else if (
    req.url.startsWith("/api/company/portfolio") &&
    req.method === "GET"
  ) {
    protectedRoute(getPortfolioProjectById)(req, res);
  } else if (
    req.url.startsWith("/api/company/portfolio-delete") &&
    req.method === "DELETE"
  ) {

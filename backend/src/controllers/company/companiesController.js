...
const createCompanyDetails = async (req, res) => {
  ...
  try {
    ...
    if (count > 0) {
      ...
      res.statusCode = 201;
      res.write(
        JSON.stringify({ message: "Company details set successfully" })
      );
    }
    res.end();
  } catch (err) {
...
const updateCompanyDetails = async (req, res) => {
  ...
  try {
    ...
    if (count === 0) {
      res.statusCode = 404;
      res.write(
        JSON.stringify({ message: "Company details not found for this user" })
      );
    } else {
      const updateQuery = `
                UPDATE companies
      ...
      res.statusCode = 200;
      res.write(
        JSON.stringify({ message: "Company details updated successfully" })
      );
    }
    res.end();
  } catch (err) {
...
const getCompanyReviews = async (req, res) => {
  ...
  try {
    ...
    if (reviewRows.length === 0) {
      res.statusCode = 404;
      res.write(
        JSON.stringify({ message: "No reviews found for this company" })
      );
    } else {
      res.statusCode = 200;
      res.write(
...
const companyCompaniesRoutes = (req, res) => {
  if (
    req.url.startsWith("/api/company/company-details") &&
    req.method === "POST"
  ) {
    let body = "";
    req.on("data", (chunk) => {
    ...
    req.url.startsWith("/api/company/company-reviews/") &&
    req.method === "GET"
  ) {

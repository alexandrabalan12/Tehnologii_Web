...
const createJobApplication = async (req, res) => {
    ...
    if (!project_id || !company_id || !message || !delivery_date || !estimated_price) {
        res.statusCode = 400;
    ...
    try {
        ...
        const [projectRows] = await database.query(selectProjectQuery, [project_id]);
        ...
        if (projectRows[0].status !== 'open') {
            ...
            res.write(JSON.stringify({ message: 'Cannot apply to this project as it is not open' }));
            res.end();
            return;
        }

        // Check if application already exists for this project by the same company
        const checkApplicationQuery = 'SELECT id FROM job_applications WHERE project_id = ? AND company_id = ?';
        const [applicationRows] = await database.query(checkApplicationQuery, [project_id, company_id]);

        if (applicationRows.length > 0) {
            res.statusCode = 409; // Conflict status code
            res.write(JSON.stringify({ message: 'Application already exists for this project' }));
            res.end();
            return;
        }
        ...
        `;
        const insertValues = [project_id, company_id, message, delivery_date, estimated_price];
        await database.query(insertQuery, insertValues);
    
        res.statusCode = 201;
        res.write(JSON.stringify({ message: 'Job application created successfully' }));
        res.end();
    } catch (err) {
        console.log('err', err);
        res.statusCode = 500;
...
const companyApplicationRoutes = (req, res) => {
    if(req.url.startsWith('/api/company/application-new') && req.method === 'POST') {
        let body = '';
        req.on('data', chunk => {
            body += chunk;
        });
    ...
    } else if(req.url.startsWith('/api/company/application') && req.method === 'GET') {
